...
// import bannerImg from "../../public/assets/lady-with-book.png";
import bannerImg from "../../public/assets/profile.png";
import ideas from "../../public/assets/ideas.webp";
      ...
      <div className="z-10 mx-auto flex w-full max-w-[1080px] flex-col space-y-8">
        <h1 className="flex max-w-2xl flex-col space-y-1 text-4xl font-bold sm:text-5xl xl:text-7xl">
          {banner_content?.title &&
            banner_content?.title.map((item, i) => <span key={i}>{item}</span>)}
        </h1>
        ...
        <div className="flex flex-row items-center space-x-10 transition">
          ...
          <div className="rounded-full bg-black p-4 group-hover:bg-black/80 hover:animate-bounce ease-in duration-300">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="currentColor"
          ...
          {/* </button> */}
          <span className="font-bold">{banner_content?.play_button}</span>
        </div>
      </div>
    </section>
  );
